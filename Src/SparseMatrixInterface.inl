...
template< class SPDFunctor, class T, typename Real, class TDotTFunctor > int SolveCG( const SPDFunctor& M, int dim, 
	ConstPointer( T ) b, int iters, Pointer( T ) x, double eps, TDotTFunctor Dot )
{
	eps *= eps;
...
#pragma omp parallel for reduction( + : delta_new ) 
	// 下面的for循环要分成多个线程跑，但每个线程都要保存变量delta_new的拷贝，循环结束后，所有线程把自己的delta_new累加起来作为最后的输出。
...
template< class SPDFunctor , class Preconditioner , class T , typename Real , class TDotTFunctor > int SolveCG( 
	const SPDFunctor& M , const Preconditioner& P , int dim , ConstPointer( T ) b , int iters , Pointer( T ) x , 
	double eps , TDotTFunctor Dot  )
{
	eps *= eps;
	Pointer( T ) r = AllocPointer< T >( dim );
	Pointer( T ) d = AllocPointer< T >( dim );
...
#pragma omp parallel for reduction( + : delta_new )
	...
	for( ii=0 ; ii<iters && delta_new>eps*delta_0 ; ii++ )
	{
		...
		Real dDotQ = 0;
#pragma omp parallel for reduction( + : dDotQ ) // 循环结束后，所有线程把自己的dDotQ累加起来作为最后的输出。
		for( int i=0 ; i<dim ; i++ ) dDotQ += Dot( d[i] , q[i] );
		if( !dDotQ ) break;
