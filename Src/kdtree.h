	...
	public:
		...
		*/
		void build(const std::vector<PointT>& points)
		{
			...
			// indices赋值为0,1,2,...,sizeof(indices)-1
		...
		*/
		...
		{
			int guess;
			double _minDist = std::numeric_limits<double>::max();  // 当前环境下double型数据的最大值
	...
	private:
		...
		*/
		Node* buildRecursive(int* indices, int npoints, int depth)  
		{
			...
			// 查找区间[indices, indices+npoints)中第indices+mid大的元素，并将其移动到indices+mid指向的位置
			// 比较规则是后面定义的匿名函数
		...
		*/
		void validateRecursive(const Node* node) const
		{
			if (node == nullptr)
				return;
			...
			if (node0)
				validateRecursive(node0);

			if (node1)
				validateRecursive(node1);
		}

		static double distance(const PointT& p, const PointT& q)  // 欧氏距离
		{
			double dist = 0;
		...
		*/
		void knnSearchRecursive(const PointT& query, const Node* node, KnnQueue& queue, int k) const
		{
			...
			const double dist = distance(query, train);
			queue.push(std::make_pair(dist, node->idx));   // 优先队列 有序存储

			const int axis = node->axis;
			const int dir = query[axis] < train[axis] ? 0 : 1;  // 二叉树
